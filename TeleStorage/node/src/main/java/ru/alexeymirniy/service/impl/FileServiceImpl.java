...
import ru.alexeymirniy.dao.AppDocumentDao;
import ru.alexeymirniy.dao.BinaryContentDao;
...
public class FileServiceImpl implements FileService {
    ...
    private byte[] downloadFile(String filePath) {
        ...
        String fullUri = fileInfoUri.replace("{token}", token).replace("{fileId}", filePath);

        URL urlOdj = null;
        try {
            urlOdj = new URL(fullUri);
        } catch (MalformedURLException e) {
            throw new UploadFileException(e);
        }

        try (InputStream is = urlOdj.openStream()) {
            return is.readAllBytes();
        } catch (IOException e) {
            throw new UploadFileException(urlOdj.toExternalForm(), e);
        }
    }
}